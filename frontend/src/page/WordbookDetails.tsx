...
const WordbookDetails = () => {
  const history = useNavigate();
    ...
    const CallAI = (word_str: string) => {
        ...
        fetch("https://fountain-of-words.noonyuu.com/app/ai?refresh=0", {
            method: "POST",
            body: JSON.stringify({
                "Text": word_str
            })
    ...
    const Call_Refresh_AI = (word_str: string) => {
        ...
        //{ message: 'スイッチは、回路内の電流の経路を変更する電気機械式デバイスです。電流をオンまたはオフしたり、回路の別の部分に経路を切り替えるために使用できます。', result: 'ok', status: 'success' }
        fetch("https://fountain-of-words.noonyuu.com/app/ai?refresh=1", {
            method: "POST",
            body: JSON.stringify({
                "Text": word_str
            })
    ...
    const Detail = (index: number) => {
        ...
        fetch("https://fountain-of-words.noonyuu.com/app/description/" + words[index]["WordID"], {
            method: "GET",
            headers: {
      ...
      <main className="h-full overflow-y-auto	bg-fixed bg-bottom pb-[30px] pt-16">
        ...
        >
          <div className="ml-4 h-5 pt-4">
            {/* TODO:仮アイコン */}
            <BsArrowReturnLeft size={24} onClick={() => history(-1)} />
          </div>
            ...
            >
              ...
              <div className="h-[70%] rounded-b-[3rem]">
                ...
                <div className="mx-10 mt-8 h-[95%] overflow-auto text-center text-2xl">
                  ...
                  <button
                    className={`${refresh_isshow ? "" : "hidden"}`}
                    onClick={() => {
